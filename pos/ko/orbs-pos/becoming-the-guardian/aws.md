...
* 최소 0.1 ETH 이상의 이더리움 코인 (노드 주소에 필요)
...
정상 동작 시 Orbs 노드는 이더리움의 PoS 컨트랙트로 자동으로 트랜잭션을 보냅니다(보상 분배 실행, 네트워크 동기화 및 위원회 진입 준비 신호 등). Orbs 노드 주소에는 이런 트랜잭션 가스를 충당할 만큼의 ETH가 있어야 하며, 가디언은 Orbs 노드 주소에 **최소 0.1 ETH** 이상이 있는지 주기적으로 확인할 책임이 있습니다.
...
4. **스택 세부 정보 지정**: 준비된 정보를 입력하세요
   1. 필수 입력 정보
      ...
      3. **Orbs node address without 0x**: 노드 주소값 중 앞에 0x 를 제외한 값입니다. (⚠️**가디언 지갑 주소 아닙니다**)  (ie. `246abE5D...000`)&#x20;
      4. **Orbs private key without 0x**: 노드 주소의 프라이빗 키 앞에 0x 를 제외한 값입니다. (⚠️**가디언 지갑 주소의 키가 아닙니다**) (ie. `0ab6F...000`)
