# Setup node using cloud

## Setup node using Cloud service (AWS)

You can use cloud service such as Amazon Web Service to obtain node infrastructure and public IP address. This can be chosen by who does not have physical space to store computer and own public IP address.
...
* Remote pc (Ubuntu or MacOS)
* AWS account
* Elastic IP address (AWS)
* Guardian Wallet (stake, rewards) - HW wallet recommended
* A private key of ERC20 address for node operation. For details see [below](setup-node-using-cloud.md#allocate-orbs-node-address-and-private-key)
* Ethereum RPC endpoint url ([Infura](https://www.infura.io/) free tier is okay)
* Knowledge of the Linux/Unix terminal
...
The generated key should **not** have a passphrase. It is okay to generate a key by any means, such as based on the following tutorial by [GitHub](https://help.github.com/articles/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent/)
...
* -C "[your\_email@example.com](mailto:your_email@example.com)" is an optional comment

## Configure AWS credentials

Create an Access Key at AWS console and save key id and access key.
...
The Node IP address and region will later be used in the node configuration file. The IP will also be required during the [Guardian Registration phase](register-your-guardian-on-chain.md).
...
{% hint style="info" %}
"Polygon" is an automated orbs-network install software.
{% endhint %}
...
Create a dedicated folder for your deployment. For example, "orbs-node" with `mkdir orbs-node` command. This folder will hold config files, logs and the artifacts for future needs (such as if you ever want to remove your AWS resources)

**Backup this folder upon completion of the instructions and DO NOT delete it.**

Change your working path `cd orbs-node` to the deployment folder and proceed with the instructions below.
...
2. update your configuration JSON file as required
3. run [polygon create](setup-node-using-cloud.md#deploy-the-node-using-polygon-cli)
